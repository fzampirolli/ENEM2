...
def main():
    ...
    filename = f"microdados_enem_{ano}.zip"
    extract_folder = os.path.join(os.getcwd(), ano)
    ...
    # 2. Extração
    print(f"Extraindo para pasta: {extract_folder}...")
    ...
    try:
        ...
        # 3. Limpeza
        print("Removendo arquivo ZIP temporário...")
        os.remove(filename)
        print(f"\n✅ Sucesso! Dados disponíveis em: {extract_folder}")

    except zipfile.BadZipFile:
        print("\n❌ Erro: O arquivo baixado está corrompido (provavelmente o download falhou ou foi bloqueado).")
        if os.path.exists(filename): 
            os.remove(filename)
