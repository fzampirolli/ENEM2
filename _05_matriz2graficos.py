...
def plot_TRI(a, b, c, D, media, mediana, std, f, TAM, i):
    """
    ...
    plt.title(f"Curva Característica do Item (CCI)", fontsize=18, fontweight='bold', color=COLOR_TEXT, pad=20)
    plt.ylabel("Probabilidade de Acerto (P)", fontsize=14, color=COLOR_TEXT)
...
def drawViolinPlot(f, vet, i, titulo_extra=""):
    """
    ...
    legenda_html = (
        ...
        f"<span style='font-size:{FONT_TITLE-5}px; font-weight:bold; color:{COLOR_TEXT}'>Questão {i}</span><br><br>"
    ...
    fig.update_layout(
        ...
        title_text=f"Distribuição de Acertos - Questão {i}",
...
def draw_signoits(output_folder, filename_base, mat, mat_raw):
    try:
    ...
    for i in tqdm(range(mat.shape[0]), desc=f"Prova {codigo}", unit="img"):
        ...
        # TRI
        fimg_tri = os.path.join(output_folder, f"{codigo}_{tam}_fig_tri_{str(i + 1).zfill(3)}.png")
        if not os.path.exists(fimg_tri):
            plot_TRI(a, b, c, D, m, med, st, fimg_tri, tam, i + 1) # Passando o número da questão

        # Violin
        if i < mat_raw.shape[1]:
            dados_item = mat_raw[:, i]
            fimg_box = os.path.join(output_folder, f"{codigo}_{tam}_fig_box_{str(i + 1).zfill(3)}.png")
            if not os.path.exists(fimg_box):
                drawViolinPlot(fimg_box, dados_item, i + 1)
...
def genStatistics(ano):
    ...
    for f_tri in files_tri:
        ...
        draw_signoits(output_dir, f_tri, mat_final, mat_respostas)
